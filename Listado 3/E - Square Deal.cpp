...
bool isSquare(pair<int, int> s1, pair<int, int> s2, pair<int, int> s3) {
	...
	if (s1.first == s2.first 
			&& s1.second + s2.second == s3.first 
			&& s1.first + s3.second == s3.first)
		return true;
	// Diff is 2
	if (s1.first == s3.first 
			&& s1.second + s3.second == s2.first 
			&& s1.first + s2.second == s2.first)
		return true;
	// Diff is 1
	if (s3.first == s2.first 
			&& s3.second + s2.second == s1.first 
			&& s2.first + s1.second == s1.first)
		return true;
